...
//session_start();
	...
	if(isset($_POST['add_follow'])){
		...
		if(!isset($_SESSION['Log']) OR !isset($_SESSION['patient_id'])){
		...
		$query = mysqli_query($con, "INSERT INTO follow_up (pat_id, admin_id, note, no_of_days, date_added) VALUES ('$pat_id', '$admin_id', '$notes', '$days', '$Date')") or die("Cannot Insert follow_up details". mysqli_error($con));
	...
	<form action="follow_up.php" method="POST">
		...
		<tbody>
			<tr>
				...
				<td><textarea name="notes" id="notes" rows="4" cols="30"></textarea></td>
			</tr>
			<tr>
